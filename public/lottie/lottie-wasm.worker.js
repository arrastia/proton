importScripts('lottie-wasm.js');
importScripts('pako-inflate.min.js');

function LottieItem(reqId, jsString, width, height, fps) {
...
LottieItem.prototype.init = function (jsString) {
  try {
    this.handle = LottieWorker.Api.init();

    this.stringOnWasmHeap = allocate(intArrayFromString(jsString), 'i8', 0);

    this.frameCount = LottieWorker.Api.loadFromData(this.handle, this.stringOnWasmHeap);

    LottieWorker.Api.resize(this.handle, this.width, this.height);
  } catch (e) {
    console.error('init LottieItem error:', e);
  }
};

LottieItem.prototype.render = function (frameNo, segmentId, clamped) {
  if (this.dead) return;
  ...
  try {
    LottieWorker.Api.render(this.handle, frameNo);

    const bufferPointer = LottieWorker.Api.buffer(this.handle);
...
LottieItem.prototype.destroy = function () {
  this.dead = true;

  LottieWorker.Api.destroy(this.handle);
};

const items = {};
const LottieWorker = (function () {
  const worker = {};
  worker.Api = {};
  ...
  loadFromData: function (reqId, url, width, height) {
    getUrlContent(url, function (err, data) {
      ...
      try {
        ...
        // }
        items[reqId] = new LottieItem(reqId, json, width, height, json_parsed.fr);
      } catch (e) {
...
function isSafari(scope) {
  if (_isSafari == null) {
    ...
    _isSafari =
      !!scope.safari || !!(userAgent && (/\b(iPad|iPhone|iPod)\b/.test(userAgent) || (!!userAgent.match('Safari') && !userAgent.match('Chrome'))));
  }
  return _isSafari;
}
